...
5. 参数经验按动作身份隔离，不能仅按 Chartcode 横向复用；
6. 操作描述中的明确事实优先于经验默认值；
7. 匹配不唯一、置信不足或数据冲突时回退，不静默套用结果；
8. 每个关键选择都写入 Trace，便于审计和人工复核。
...
单条输入同样先经过主体判定、拆解、翻译、父工序重量分组，再逐条分析子工序。
...
- 最终 Excel 的 Decisions、Chart、C/V、Freq、Time 字段统一输出 `NA`。
...
当前实现把父工序和全部子工序一次性交给 LLM，识别“同一个物理零件”的连续操作链。
