...
→ 父工序内重复动作归一并共享一次决策
→ scoped T0 或上传 Common Chart 关键词/语义快速决策（无零件身份且无可靠重量时）
...
Common 关键词零候选时的语义回退；关闭后 Common 的确定性关键词匹配仍继续工作。
...
flowchart TD
    ...
    T0 -->|未命中| T05["T0.5 Common：关键词优先，零词法候选时语义 Top1"]
...
### C8. Common 关键词优先、语义回退

Common 使用三级顺序：

```text
扫描全部候选的归一化精确匹配
→ 若全局没有精确项，再做单向 keyword in input
→ 包含项选择最长关键词
→ 词法层完全没有候选时，语义 Top1@0.70
```

禁止用 `input in keyword` 反向召回。精确或包含层一旦出现候选，就不再进入语义层：

- 相同最高优先级指向等价输出：固定选择 Excel 行号最小、关键词位置最靠前的一项；
- 相同最高优先级指向不同输出：返回歧义并进入后续 Resolver，语义不得覆盖歧义。

只有关键词层完全没有候选，且语义开关开启时，才对全部 Common 语义文档取 Top1。
阈值为 `0.70`，不要求 margin，同分按 Common Excel 行号稳定选择。
...
此时 Chartcode 经验回退选码 LLM；Common 保留关键词结果，若关键词也未命中则继续后续
...
1. 全候选精确关键词优先；无精确项时执行单向最长包含；
2. 词法层完全没有候选且语义开关开启时，执行 Common 语义 Top1，阈值 `0.70`、无
   margin、同分按 Excel 行号稳定选择；
3. 关键词候选若因不同输出产生歧义，立即停止 Common，禁止语义覆盖；
4. 普通行严格解码决策串，并用当前数据库公式重算时间；
5. `EST C00` / `EST V00` 行直接使用已校验的上传单次时间；
6. 命中结果以 `freq=1` 模板写入当前 scope，输出时只乘输入记录 `freq` 一次。

语义关闭或向量不可用时仍执行步骤 1；没有关键词但“操作内容”有效的 Common 行，仅在
语义层参与匹配。
...
| Common 关键词最高优先级存在不同输出 | 不使用 Common，继续后续链路 |
| Common 完全没有关键词候选 | 语义开启时尝试 Top1@0.70；否则继续后续链路 |
...
→ Common 关键词全候选 exact；否则最长单向 contains
→ 关键词零候选时语义 Top1@0.70；关键词歧义禁止语义覆盖
