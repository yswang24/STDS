...
→ 未命中时把全部有效 Chartcode 经验交给 LLM 选一行
→ 经验未选中/非法/异常时由 LLM 从数据库普通 Chartcode 列表选码
...
13. 动作缓存以完整工作簿摘要、Common 开关和经验辅助开关隔离；存在零件身份时不读写动作缓存；
...
UI 与 API 的 `use_semantic_experience` 默认开启。字段名为兼容现有契约而保留，当前表示
“启用上传经验辅助选码”，并允许 Common 语义 Top1 优先匹配；关闭后跳过 Chartcode
经验 LLM 选择，Common 直接执行确定性关键词匹配。
...
flowchart TD
    ...
    WEIGHTSCOPE -->|否| T0["T0：摘要 + Common开关 + 经验辅助开关"]
    ...
    T2 -->|设备| OUTPUT
    T2 -->|人工| T3E["T3E 全部有效 Chartcode 经验交给 LLM 选一行"]
    T3E -->|选中| CHART["确定 Chartcode 与经验身份"]
    T3E -->|未选中/非法/异常或开关关闭| T3["T3 LLM 从数据库普通 Chartcode 选择"]
    T3 --> CHART

    CHART --> PARAMEXP["经验行选码绑定同身份；数据库普通选码后整条选择参数经验"]
...
- Chartcode 来自全量经验 LLM 所选行：只能读取与该选码行完全相同经验身份的参数规则；
- Chartcode 来自数据库普通列表 LLM：收集该 Chartcode 下所有有效参数经验，随后一次选择一整条。
...
保留，并以“操作内容”单元格作为语义候选。有效的“关键词描述1～8”也各自成为独立候选，
不与操作内容拼接。
...
### C7. Chartcode 经验全量交给 LLM 选择

生产选码路径不再对 Chartcode 经验做向量 Top1，也不使用操作内容的精确或包含匹配直接
决定 Chartcode。`use_semantic_experience=true` 时，索引按 `chart_row`（Excel 行号）、
经验 ID 和规范操作内容稳定排序，将全部有效 Chartcode 经验行一次性交给 LLM：

```text
全部有效 Chartcode 经验行（不按 Chartcode 去重）
→ LLM 结合当前动作选择一行
→ 返回有效行索引：接受该行的 Chartcode 和完整经验身份
→ 未选择/非法索引/调用异常：回退数据库普通 Chartcode LLM
```

不按 Chartcode 去重是必要约束：例如“转身”和“弯腰”可以共用同一个码，但所选经验行
必须携带不同的动作身份，后续才能严格绑定各自参数。当前示例文件的 41 条有效经验会作为
41 个候选一次性传给 LLM，因此相较旧向量 Top1 增加一次 LLM 调用，但此路径不需要向量库
或 embedding 服务。`use_semantic_experience=false` 时直接跳过该经验 LLM，进入数据库普通
Chartcode 选择。

### C8. Common 语义 Top1 优先、关键词回退

Common 以单元格为向量候选。每一行的“操作内容”和每个有效“关键词描述1～8”分别建立向量；
决策描述、Chartcode、C/V、频率、时间等结果字段不向量化。随后默认先执行语义 Top1：

```text
输入与全部有效 Common 单元格向量比较
→ 取全局 cell Top1，score >= 0.70 时按所属 Excel 行回取整行决策
→ 低分、语义关闭或向量不可用时，扫描归一化精确关键词
→ 若全局没有精确项，再做单向 keyword in input，并选择最长关键词
```

语义 Top1 不要求 margin。同分先按 Common Excel 行号，再按字段顺序“操作内容 → 关键词描述1～8”
稳定选择。达标的语义结果优先于
...
1. Chartcode 由全量经验 LLM 选出时，使用该选码行的经验 ID、归一化操作内容和 Chartcode
   严格查找同身份参数。不得切换到同码的另一动作；同身份已有参数提示则直接沿用，唯一
   绑定行可用，多条绑定冲突则全部忽略；
2. Chartcode 由数据库普通列表 LLM 选出时，取该码下全部有效参数经验。零条则不用经验，一条直接用，
...
Common 使用当前上传上下文内的内存向量索引。单元格向量在第一次真正需要语义匹配时才
构建；同一索引的并发首次查询通过异步锁合并，不需要持久化向量数据库。Chartcode 经验
全量 LLM 选择只读取有效经验行，不构建或查询向量。Common 查询会在索引级隔离远端后端
共享的成功/降级状态，避免并发失败把 Mock 占位向量误判为有效语义向量。
...
此时 Common 回退关键词匹配，若关键词也未命中则继续后续 Resolver 链路。降级不会用
伪向量猜测 Common；Chartcode 经验全量 LLM 选择不受向量降级影响。
...
cache_scope = 完整工作簿摘要 + Common开关 + 经验辅助开关
```
...
- Common 与经验辅助开关状态都进入缓存 scope，任一开关切换后不能命中另一模式留下的结果。
...
1. 经验辅助开关开启且向量可用时，将输入与全部“操作内容/有效关键词”单元格向量比较，
   取全局 cell Top1；阈值 `0.70`、无 margin，同分按 Excel 行号和字段顺序稳定选择，达标时回取整行；
...
没有关键词但“操作内容”有效的 Common 行仍以该操作内容单元格参与语义层；
语义未命中时没有关键词可供回退。
...
### T3E. Chartcode 经验全量 LLM 选择

经验辅助开关开启时，人工动作读取 `ExperienceIndex.chartcode_contexts()`，将全部有效选码
经验行一次性交给 `llm_select_chartcode_experience`：

```text
动作描述 + 全部有效选码经验行 → LLM 选择一行
有效索引 → 接受 Chartcode 与该行完整经验身份
未选择 / 非法索引 / 调用异常 → T3 数据库普通 Chartcode LLM
```

候选按 Chart Excel 行号稳定排序，不按 Chartcode 去重。选中后还必须确认 Chartcode 存在
于当前图表库，并保留完整经验身份、候选总数、选择索引和 LLM 理由，用于严格绑定参数与 Trace。

### T3. LLM 选择 Chartcode

经验辅助关闭，或全量经验 LLM 没有返回有效行时，LLM 从当前数据库普通图表库选择
Chartcode；EST 固定时间码不进入该普通候选集。

最终参数经验选择严格区分选码来源：

- 全量经验 LLM 选码：只读取同一 `experience_id + operation + chartcode` 身份绑定的参数，绝不
  换到同码另一动作；
- 数据库普通 Chartcode LLM 选码：获取该码全部有效参数经验；单条直接用，多条一次性交给
...
语义命中时额外写入 cell（命中字段）、cell_text（命中单元格内容）和 row（所属 Excel 行）
...
以及“经验行同身份绑定 / 数据库普通选码后唯一候选 / 多候选 LLM 整条选择”等模式。
...
复核缓存和 Resolver 使用同一个 scope：完整工作簿摘要 + Common 开关 + 经验辅助开关。
...
| Chartcode 经验 LLM 未选择、返回非法索引或调用异常 | 回退数据库普通 Chartcode LLM |
| 上传经验辅助选码关闭 | 跳过 Chartcode 经验 LLM；Common 仅使用关键词 |
| 全量经验 LLM 选码但同身份参数绑定冲突 | 不换用同码其他动作，忽略参数经验 |
| 数据库普通 Chartcode LLM 选码后有多条同码参数经验 | 一次交给 LLM 选择整条；失败或非法索引则不用参数经验 |
| Common 未上传或没有有效行但开关开启 | UI 暂停；API 返回 422 |
| Common 单元格语义 Top1 达到 0.70 | 按命中 cell 所属 Excel 行回取整行，即使关键词层存在其他候选或歧义 |
...
- Common 或经验辅助开关切换都会生成不同 scope，不会命中另一模式的结果；
...
数据库普通 Chartcode 选码和历史 kNN 过滤 EST，避免把固定时间类型当作一般动作图表。
EST 只能通过经过校验的上传经验显式进入：由 `Common_Chart` 命中，或由全量 Chartcode
经验 LLM 明确选中。
...
- Common 语义向量按请求级上下文在首次使用时懒加载，只保存在内存中，不需要持久化向量库；
...
→ 无零件身份且无可靠重量时读取 scoped T0（完整摘要 + Common开关 + 经验辅助开关）
...
Chartcode
全部有效选码经验 → LLM 选择一行
→ 未选择/非法/异常时由 LLM 从数据库普通 Chartcode 列表选择

参数经验
全量经验 LLM 选中某行 → 严格绑定同一经验身份，不换到同码另一动作
数据库普通 Chartcode LLM 选码 → 同码参数经验零条不用、一条直用、多条由 LLM 一次选择整条
→ 唯一选定记录贯穿全部 Vn，禁止混用
...
| Chartcode 经验候选池 | `stds/experience/index.py` |
| 经验数据契约 | `stds/experience/models.py` |
| 全量 Chartcode 经验 LLM 选择 | `stds/llm/select_chartcode_experience.py` |
