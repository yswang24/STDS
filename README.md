...
| 字段 | 说明 |
|---|---|
| 操作内容 | Common 动作名称；该单元格独立建立向量，也用于展示和审计 |
...
| 关键词描述1～8 | 每个有效关键词单元格分别建立向量，并用于确定性回退；可为空，此时该行仍可按“操作内容”单元格参与语义 Top1 |
...
flowchart TD
    ...
    WEIGHTSCOPE -->|否| T0["按工作簿摘要 + Common/经验辅助开关读取 scoped T0"]
    T0 -->|未命中| COMMON["Common：语义 Top1@0.70 优先，低分/不可用时回退关键词"]
    COMMON -->|未命中且上传上下文未启用选码/参数经验池| T1["T1 历史 kNN（可选）"]
    COMMON -->|未命中且有选码/参数经验| EXPCHART
    WEIGHTSCOPE -->|是| EXPCHART["跳过 T0/Common/T1，进入选码"]
    T1 -->|未命中| EXPCHART
    COMMON -->|普通命中| FORMULA["严格解码并用数据库公式重算"]
    COMMON -->|EST 命中| FIXED["直接使用上传固定时间"]
    T0 -->|命中| OUTPUT
    EXPCHART -->|经验辅助开启| EXPLLM["全部有效 Chartcode 经验交给 LLM 选一行"]
    EXPCHART -->|经验辅助关闭或未选中| DBLLM["LLM 从数据库普通 Chartcode 选择"]
    EXPLLM -->|选中| PAREXP["严格绑定所选经验行的参数经验"]
    EXPLLM -->|未选中/非法/异常| DBLLM
    DBLLM --> PAREXPDB["收集同码参数经验并整条选择"]
    PAREXP --> TRAVERSE
    PAREXPDB --> TRAVERSE
    TRAVERSE["按 Vn/Range 遍历决策树"]
...
- C/V、时间、频率和关键词均逐行验证；单个无效关键词会被忽略；如果该行其他字段有效、
  “操作内容”非空，即使没有任何有效关键词也保留该行，使其可以参与 Common 语义 Top1。
...
### Step 9：上传经验辅助选码与 Common 匹配

UI 和 API 的 `use_semantic_experience` 默认开启。为兼容已有 API，该字段名保持不变；
当前它表示“启用上传经验辅助选码”，并同时允许 Common 使用语义 Top1。关闭后跳过
Chartcode 经验 LLM 选择，Common 直接执行确定性关键词匹配；上传文件中的参数经验仍然
保留。该开关也进入 UI 运行签名，切换后旧批量/单条结果不会继续作为当前配置的结果展示。

Chartcode 选择经验不再做向量 Top1，也不按操作内容做精确/包含匹配。Common 完整未命中后，
系统把当前文件中的全部有效 Chartcode 经验行一次性交给 LLM，让模型选择一行；当前示例
文件是 41 条候选。这里保留每一行而不按 Chartcode 去重，因为多个动作可能使用同一个码，
所选动作身份还要严格绑定后续参数经验。若 LLM 未选择、返回非法索引或调用异常，系统再让
LLM 从数据库普通 Chartcode 列表中选择。全量经验选码会增加一次 LLM 调用，但本身不需要
向量库或 embedding 服务。

Common 默认先执行语义 Top1：

```text
每行的“操作内容”和每个有效“关键词描述1～8”分别建立向量
→ 输入与全部 Common 单元格向量比较，取全局 cell Top1（阈值 0.70）
→ 达标：按命中单元格所属的 Excel 行回取整行 Common 决策
→ 低分、语义关闭或向量不可用：归一化精确关键词
→ 没有精确项时，单向 keyword in input 的最长包含
```

Common 不再把同一行拼成一个语义文档。“操作内容”和每个有效关键词都是独立候选单元格；
决策描述、Chartcode、C/V、频率和时间等结果字段不向量化。没有有效关键词但操作内容有效的行，
仍会生成一个“操作内容”候选。语义 Top1 不要求 margin；同分先按 Common Excel 行号，再按字段顺序
“操作内容 → 关键词描述1～8”稳定选择。达标结果优先于关键词结果，也可覆盖关键词歧义。
只有语义未命中后才进入关键词回退；若关键词
最高优先级指向不同输出，则回退后续 Resolver 链路。系统也不会使用 `input in keyword`
的反向包含。

Common 的向量在当前上传请求的内存索引中首次需要时懒加载；并发首次请求会合并构建，
不需要持久化向量数据库。Mock embedding 或远端 embedding 失败后降级得到的占位向量
不会产生 Common 语义命中，而是回退关键词；并发查询会隔离共享后端的成功/降级状态，
避免把 Mock 占位向量误判成语义命中。Chartcode 经验全量 LLM 选择不依赖该索引。
...
1. 上传经验辅助选码开启时，将全部有效 Chartcode 经验行交给 LLM 选择一行；
2. 没有经验候选、LLM 未选择、返回非法索引、调用异常或开关关闭时，调用 LLM 从当前
   数据库普通 Chartcode 列表选择；
3. 若 Chartcode 来自所选经验行，参数只能读取同一经验身份绑定的规则。即使同一 Chartcode
   下还有“转身/弯腰”等其他动作，也不能切换到另一身份；同一身份出现多条冲突规则时
   全部忽略，避免混用；
4. 若 Chartcode 来自数据库普通列表 LLM，收集该码下全部有效参数经验：没有则沿用普通参数逻辑，只有
...
因此，Chartcode 来自数据库普通列表时参数经验仍全局生效，但“全局”表示所有同码候选
都进入一次整条选择，不表示各 Vn 可以从不同动作经验中自由取值。
...
→ 人工/设备判定
→ 全部有效 Chartcode 经验交给 LLM 选一行
→ 未选中/非法/异常时由 LLM 从数据库普通 Chartcode 列表选择
→ 按选码来源绑定/整条选择参数经验
→ 决策树遍历与公式计算
```
...
T0 的 scope 由“完整上传工作簿 SHA-256 摘要 + Common 开关 + 经验辅助开关”共同组成。
...
拆分出的多条记录仍各自是单次结果，不会把组成员数写成 `×N`。系统内部仍保留 EST
固定时间决策值；最终 XLSX 的 `Decisions` 单元格仅作展示留空，`Time(s)` 正常输出，
不会影响工时计算。CSV 和页面预览仍保留内部决策值。
...
Chartcode：
全部有效选码经验 → LLM 选择一行 → 未选择/非法/异常则从数据库普通 Chartcode 列表 LLM 选择

完整动作快速路径：
无零件身份且无可靠重量时，按“完整工作簿摘要 + Common开关 + 经验辅助开关”读取 scoped T0
→ 上传 Common_Chart：语义 Top1@0.70 优先
→ 低分/关闭/向量不可用时回退关键词 exact → 最长单向 contains
→ 回退关键词存在输出歧义时继续后续链路
→ 仅在上传上下文未启用选码/参数经验池时尝试 T1

参数：
全量经验 LLM 选中某行 → 严格绑定该经验身份
数据库普通 Chartcode LLM 选码 → 收集同码全部参数经验，单条直用，多条由 LLM 一次选择整条
→ 选中记录贯穿全部 Vn，禁止跨记录混用
...
- 上传 `Common_Chart` 中的 EST 行按固定时间类型处理，直接以上传时间/源频率得到单次
  时间，不依赖普通公式重算。
- 内部仍保留固定时间决策值；最终 XLSX 的 `Decisions` 对 EST 留空，`Time(s)` 正常输出，
  不影响计算。CSV 和页面预览仍保留该内部值。

数据库普通 Chartcode 选码与历史检索过滤 EST；EST 只能通过经过严格校验的上传经验
显式进入：由 `Common_Chart` 命中，或由全量 Chartcode 经验 LLM 明确选中，避免固定
时间类型被普通路径误选。
...
- 零件和 Common 语义索引均保存在请求级内存中并按需懒加载；Common 索引以单元格为向量候选，没有持久化向量库；
- Mock embedding 或远端 embedding 已降级为 Mock 时不会产生零件或 Common 语义命中；
...
时，整批结果会被标记为不可用于语义决策；Common Chart 和零件重量不会把降级 Mock
向量当成真实匹配。Chartcode 经验全量选码由 LLM 完成，不依赖向量服务。
