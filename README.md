...
| 关键词描述1～8 | Common 语义文档和确定性回退关键词；可为空，此时仍可按“操作内容”参与语义 Top1 |
...
以及 Common 的语义 Top1 优先匹配；关闭它后 Common 直接执行确定性关键词匹配，也不会
删除上传文件中的参数经验。该开关也进入 UI 运行签名；切换后旧批量/单条结果不会继续
作为当前配置的结果展示。
...
仍参加语义 Top1。语义 Top1 不要求 margin，同分按 Common 的 Excel 行号稳定选择；达标
结果优先于关键词结果，也可覆盖关键词歧义。只有语义未命中后才进入关键词回退；若关键词
